...
describe("userManager", () => {
  ...
  describe("addFavoriteUser", () => {
    it("should add a new favorite user", async () => {
      ...
      const addedUser = Object.values(result).find(
        (u) => u.user.username === "johndoe"
      );
    ...
    it("should throw an error if user already exists", async () => {
      ...
      await expect(userManager.addFavoriteUser(user)).rejects.toThrow(
        "User already exists"
      );
    });
    ...
    it("should throw an error if maximum number of users is reached", async () => {
      ...
      await expect(
        userManager.addFavoriteUser({ username: "user6" })
      ).rejects.toThrow("Maximum number of users reached");
    });
  });
  ...
  describe("getUserByUsername", () => {
    ...
    it("should throw an error if user is not found", async () => {
      await expect(
        userManager.getUserByUsername("nonexistent")
  ...
  describe("deleteFavoriteUser", () => {
    ...
    it("should throw an error if user is not found", async () => {
      await expect(
        userManager.deleteFavoriteUser("nonexistent")
      ).rejects.toThrow("User not found");
    });
    ...
    describe("toggleStar", () => {
      ...
      it("should throw an error if user is not found", async () => {
        await expect(userManager.toggleStar("nonexistent")).rejects.toThrow(
          "User not found"
        );
      });
